...
**このドキュメントの対象**: `lib/posts.js` の `renderChartHtml` から呼ばれる各chart type(bar/donut/lineChart/prosCons/compareCards等)(記事専用モジュール `lib\*Widgets.js`/`*Extras.js` は生活サイトでは全廃済み。新規追加も禁止)の図解・グラフ描画関数。
