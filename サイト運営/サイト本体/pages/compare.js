...
export async function getStaticProps() {
  ...
  // 悩み(frontmatterのworry)の選択肢は記事データから動的に集約する
  const worries = [...new Set(items.flatMap((item) => item.worry || []))].sort((a, b) =>
    a.localeCompare(b, "ja")
  );
  return { props: { items, categories, worries } };
}

export default function ComparePage({ items, categories, worries }) {
  const router = useRouter();
  const [category, setCategory] = useState("");
  const [worry, setWorry] = useState("");
  const worryItems = worries.map((w) => ({ slug: w, label: w }));

  // /compare?category=掃除 のようなURLクエリを初期絞り込み状態として反映する。
  useEffect(() => {
  ...
  return (
    <Layout
      title="商品・サービス比較 | 生活を豊かにする総合ガイド｜NEVORA"
      description="生活を豊かにする総合ガイド｜NEVORAで紹介している商品・サービスを一覧で比較できるページです。"
