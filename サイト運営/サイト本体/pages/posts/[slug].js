...
// 本文HTML(post.contentHtml)を使わず専用コンポーネントで描画する記事のslug一覧。
// 現在は該当なし(空)。必要になった記事が出たらここに追加する。
...
function buildPostJsonLd(post, siteUrl) {
  if (!siteUrl) return [];
  // slugは日本語を含むため、sitemap.xml(<loc>)と同じくpercent-encodedで揃える
  // (未エンコードのまま出力するとサイト側が返すURLと文字列が一致しない。
  //  pages/sitemap.xml.js の同趣旨のコメント・2026-08-24の全記事404の経緯を参照)。
...
export default function PostPage({ post, related, nextPost }) {
  ...
  const isCustomLayout = CUSTOM_LAYOUT_SLUGS.has(post.slug);
  ...
  return (
    ...
    >
      ...
      <article>
        ...
        */}
        {!isCustomLayout && !post.hasEmbeddedToc && <ArticleToc items={post.toc} />}

        {!isCustomLayout && post.affiliateLinks?.length > 0 && (
        ...
        {/*
          ...
          カスタムレイアウト記事は本文内で全アフィリエイトリンクを自前配置済みのため対象外。
        */}
        {!isCustomLayout && post.unplacedAffiliateLinks?.length > 0 && (
        ...
        {!isCustomLayout && related.length > 0 && (
